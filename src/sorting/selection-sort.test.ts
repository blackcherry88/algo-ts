...
describe('selectionSort', () => {
  ...
  it('supports a descending comparator', () => {
    expect(selectionSort([1, 3, 2], (a, b) => b - a)).toEqual([3, 2, 1]);
  });

  it('sorts objects using a custom comparator', () => {
    const small = { value: 1 };
    const large = { value: 2 };
    expect(selectionSort([large, small], (a, b) => a.value - b.value)).toEqual([
      small,
      large,
    ]);
  });
  ...
  it('sorts strings with the default comparator', () => {
    expect(selectionSort(['pear', 'apple', 'banana'])).toEqual([
      'apple',
      'banana',
      'pear',
    ]);
